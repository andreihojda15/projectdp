...
function Groups({ batch }) {
  ...
  return (
    <div className="w-4/5 justify-center items-center text-center">
      {modal && <GroupsModal modal={modal} toggleModal={toggleModal} />}

      <div className="flex flex-row justify-between items-center">
        <a href="/groups"><p className="font-bold text-3xl lg:text-left p-2">Groups</p></a>
        <svg
          className="w-6 h-6"
          fill="none"
          stroke="currentColor"
      ...
      <div className="grid xl:grid-cols-4 md:grid-cols-2 gap-4 justify-center items-center">
        {groups.slice(page * batch, (page + 1) * batch).map((group) => {
          ...
          return (
            <Link to="/group" className="bg-slate-700 text-white rounded flex flex-col" key={group}>
              {group}
...
function GroupsModal({ modal, toggleModal }) {
  return (
        ...
        <div className="relative p-4 w-full max-w-md h-full md:h-auto">
          <div className="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div className="py-6 px-6 lg:px-8">
              <h3 className="mb-4 text-xl font-medium text-gray-900 dark:text-white">Create a new Group</h3>
              <form className="space-y-6" action="#">
                <div>
                  <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                    <span className="p-4">Group Name</span>
                ...
                <div>
                  <label className="block mb-2 text-sm font-medium text-gray-900 dark:text-gray-300">
                    <span className="p-4">Group Description</span>
